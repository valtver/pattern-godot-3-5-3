...
func InitTutorialScreens():
	pages = []
	for child in $Pivot/TutorialBaseSheet.get_children():
		if child is Spatial:
			pages.push_back(child)
			
	get_node("Pivot/TUTORIAL_PAGE_TITLE").text = tr("TUTORIAL_PAGE_TITLE")
	pass
	
func ShowNextTutorialScreen():
	var nextPage = currentVisible + 1
	var found = false
	for page in pages:
		page.visible = false
